...
// Sprawdzam, czy skrypt wczytał się poprawnie
...
// Na start ukrywam loader i komunikat o błędzie
loaderEl.style.display = 'none';
errorEl.style.display = 'none';
// Pokazuję powitanie w sekcji info
infoEl.textContent = "Witaj w aplikacji do wyszukiwania kraj\xf3w!";
// Opóźnienie dla debounce (300 ms)
const DEBOUNCE_DELAY = 300;
// Referencje do elementów DOM, z którymi pracujemy
const elements = {
    searchInput: document.querySelector('#search-box'),
    countryList: document.querySelector('.country-list'),
    countryInfo: document.querySelector('.country-info')
};
// Funkcja czyszcząca zawartość wskazanego elementu
...
function onInputHandler(event) {
    ...
    if (!query) {
        clearContent(elements.countryList);
        clearContent(elements.countryInfo);
        return;
    }
    (0, _fetchCountriesJs.fetchCountries)(query).then((data)=>{
        if (data.length > 10) {
            (0, _notiflix.Notify).info("Znaleziono za du\u017Co wynik\xf3w. Prosz\u0119 poda\u0107 bardziej szczeg\xf3\u0142ow\u0105 nazw\u0119.");
            return;
        }
        displayResults(data);
    }).catch((err)=>{
        clearContent(elements.countryList);
        clearContent(elements.countryInfo);
        (0, _notiflix.Notify).failure('Ups, nie znaleziono kraju o takiej nazwie.');
    });
}
// Funkcja wyświetlająca wyniki - lista lub pojedynczy kraj
function displayResults(data) {
    if (data.length === 1) {
        clearContent(elements.countryList);
        const markup = createCountryInfoMarkup(data[0]);
        elements.countryInfo.innerHTML = markup;
    } else {
        clearContent(elements.countryInfo);
        const markup = createCountryListMarkup(data);
        elements.countryList.innerHTML = markup;
    }
}
// Tworzenie markup listy krajów (dla wielu wyników)
function createCountryListMarkup(countries) {
    return countries.map(({ name, flags })=>{
        return `
        <li>
          <img src="${flags.svg}" alt="Flaga ${name.official}" width="60" />
          <span>${name.official}</span>
        </li>
      `;
    }).join('');
}
// Tworzenie markup szczegółów pojedynczego kraju
function createCountryInfoMarkup({ name, capital, population, flags, languages }) {
    return `
    <div>
      <img src="${flags.svg}" alt="Flaga ${name.official}" width="200" />
      <h1>${name.official}</h1>
      <p><strong>Stolica:</strong> ${capital}</p>
      <p><strong>Ludno\u{15B}\u{107}:</strong> ${population}</p>
      <p><strong>J\u{119}zyki:</strong> ${Object.values(languages).join(', ')}</p>
    </div>
  `;
}
// Podpinam obsługę inputa z debouncem, żeby zmniejszyć liczbę zapytań
elements.searchInput.addEventListener('input', (0, _lodashDebounceDefault.default)(onInputHandler, DEBOUNCE_DELAY));

},{"./fetchCountries.js":"bCGdT","lodash.debounce":"irvaP","notiflix":"hpqv0","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"bCGdT":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
...
},{}],"hpqv0":[function(require,module,exports,__globalThis) {
var global = arguments[3];
...
},{}]},["cRH9c","a0t4e"], "a0t4e", "parcelRequired7c6", {})

//# sourceMappingURL=goit-js-hw-10.31b563d9.js.map
